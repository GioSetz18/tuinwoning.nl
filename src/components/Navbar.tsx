    ...
    >
      <div className="container-custom flex justify-between items-center">
        <Link to="/" className="flex items-center">
          <span className="text-xl font-bold text-sage">Tuinwoning.nl</span>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-6">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              to={link.path}
              className="text-charcoal hover:text-sage transition-colors font-medium text-sm"
            >
              {link.name}
            </Link>
          ))}
            ...
            <nav className="flex flex-col gap-3">
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  to={link.path}
                  className="text-charcoal hover:text-sage transition-colors py-2 font-medium"
